            ...
            @foreach($surkel as $item)
                <tr class="item{{$item->id}}">
                    ...
                    @if ($item->status == 2)
                        ...
                        @php
                            $status="Menunggu Konfirmasi Pimpinan";
                        @endphp
                    ...
                    @else
                        ...
                        @php
                            $status="Menunggu Konfirmasi Bagian Umum";
                        @endphp
                    @endif
                    ...
                    @php
                        $dari = App\Http\Controllers\SuratKeluarController::descJabatan($item->dari);
                    @endphp

                    <td>
                        <button class="edit-modal3 btn btn-info" 
                            data-info="{{$item->nomor}}*{{$item->kepada}}*{{$dari}}*{{$item->tembusan}}*{{$item->tanggal}}*{{$item->sifat}}*{{$item->lampiran}}*{{$item->hal}}*{{$item->softfile}}*{{$status}}*{{$item->id}}">
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <div class="deleteContent">
                        Are you Sure you want to delete <span class="dname3"></span> ? 
                        <span class="hidden did3"></span> <!-- id untuk delete diambil dari sini -->
                    </div>
